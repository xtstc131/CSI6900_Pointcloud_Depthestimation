...
from models.pointnet2_cls_msg import PointnetBins
...
class DecoderBN(nn.Module):
    def __init__(self, num_features=2048, num_classes=1, bottleneck_features=2048):
        ...
        self.conv2 = nn.Conv2d(bottleneck_features,
                               features, kernel_size=1, stride=1, padding=1)

        self.up1 = UpSampleBN(skip_input=features // 1 +
                              112 + 64, output_features=features // 2)
        self.up2 = UpSampleBN(skip_input=features // 2 +
                              40 + 24, output_features=features // 4)
        self.up3 = UpSampleBN(skip_input=features // 4 +
                              24 + 16, output_features=features // 8)
        self.up4 = UpSampleBN(skip_input=features // 8 +
                              16 + 8, output_features=features // 16)
...
class UnetAdaptiveBinsPointcloud(nn.Module):
    # n_bins -> N
    def __init__(self, backend, n_bins=100, min_val=0.1, max_val=10, norm='linear'):
        super(UnetAdaptiveBinsPointcloud, self).__init__()
        ...
        self.pointnet = PointnetBins(n_bins = n_bins,normal_channel=False)
    ...
    def forward(self, x, y,**kwargs):
        ...
        # bin_widths_normed -> bin_widths_pointcloud
        range_attention_maps = self.adaptive_bins_layer(
            unet_out)
        bin_widths_normed,_ = self.pointnet(y)
...
if __name__ == '__main__':
    model = UnetAdaptiveBinsPointcloud.build(100)
